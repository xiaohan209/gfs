...
func (nm *namespaceManager) lockParents(p mygfs.Path, goDown bool) ([]string, *nsTree, error) {
...
func (nm *namespaceManager) PartionLastName(p mygfs.Path) (mygfs.Path, string) {
	for i := len(p) - 1; i >= 0; i-- {
		if p[i] == '/' {
...
func (nm *namespaceManager) Create(p mygfs.Path) error {
	var filename string
...
func (nm *namespaceManager) Delete(p mygfs.Path) error {
	...
	cwd.children[mygfs.DeletedFilePrefix+filename] = node
	return nil
}

// Rename rename an file on path p.
func (nm *namespaceManager) Rename(source, target mygfs.Path) error {
	log.Fatal("Unsupported Rename")
	return nil
}

// Mkdir creates a directory on path p. All parents should exist.
func (nm *namespaceManager) Mkdir(p mygfs.Path) error {
	var filename string
	p, filename = nm.PartionLastName(p)
...
func (nm *namespaceManager) List(p mygfs.Path) ([]mygfs.PathInfo, error) {
	log.Info("list ", p)
	...
	ls := make([]mygfs.PathInfo, 0, len(dir.children))
	for name, v := range dir.children {
		ls = append(ls, mygfs.PathInfo{
			Name:   name,
			IsDir:  v.isDir,
			Length: v.length,
			Chunks: v.chunks,
